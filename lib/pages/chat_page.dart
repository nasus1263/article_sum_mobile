...
class ChatPage extends StatefulWidget {
  ...
  const ChatPage({super.key, this.initialContentId, this.initialRequestSeq = 0});
...
class _ChatPageState extends State<ChatPage> {
  ...
  void didUpdateWidget(covariant ChatPage oldWidget) {
    ...
    if (widget.initialContentId != null && widget.initialRequestSeq != oldWidget.initialRequestSeq) {
  ...
  Future<void> _refreshSessionList() async {
    final summaries = await ChatStore.listSessions();
    if (!mounted) return;
  ...
  Future<void> _openSession(int id) async {
    ...
    final session = await ChatStore.getSession(id);
  ...
  Future<void> _handleSend() async {
    ...
    final userMsg = ChatMessage(role: 'user', content: text, createdAt: DateTime.now().toIso8601String());
    setState(() {
    ...
    await ChatStore.setProvider(id, _provider);
    _refreshSessionList();
    ...
    try {
      ...
      await ChatStore.appendMessage(id, assistantMsg);
      if (!mounted || _selectedId != id) return;
      setState(() {
        _session = ChatSession(
  ...
  Future<void> _confirmDeleteSession(int id) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Delete this conversation?', style: TextStyle(color: AppColors.slate100)),
        actions: [
          TextButton(
  ...
  Widget _buildSessionList() {
    ...
    return ListView.separated(
      ...
      separatorBuilder: (_, _) => const Divider(height: 1, color: AppColors.slate800),
      itemBuilder: (context, i) {
        ...
        return ListTile(
          ...
          leading: article.data.thumbnail != null
              ? ClipRRect(
                  ...
                  child: Image.network(
                    ...
                    width: 44,
                    fit: BoxFit.cover,
                    errorBuilder: (_, _, _) => Container(height: 44, width: 44, color: AppColors.slate800),
                  ),
                )
              : Container(
                  height: 44,
                  width: 44,
                  decoration: BoxDecoration(color: AppColors.slate800, borderRadius: BorderRadius.circular(8)),
                ),
          title: Text(
            article.data.title ?? article.data.category ?? 'Article',
            style: const TextStyle(color: AppColors.slate100, fontSize: 14, fontWeight: FontWeight.w500),
            maxLines: 1,
          ...
          trailing: IconButton(
            icon: const Icon(Icons.delete_outline, color: AppColors.slate500, size: 20),
      ...
      children: [
        Container(
          ...
          decoration: const BoxDecoration(
            border: Border(bottom: BorderSide(color: AppColors.indigo500, width: 2)),
          ),
          child: Row(
            children: [
              IconButton(
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      article.url,
                      style: const TextStyle(color: AppColors.indigo400, fontSize: 11),
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                    ),
                  ],
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              ...
              if (_sending)
                _MessageBubble(
                  message: ChatMessage(role: 'assistant', content: _streamingText.isEmpty ? '...' : _streamingText, createdAt: ''),
                ),
              if (_error != null)
                Container(
                  ...
                  child: Text(_error!, style: const TextStyle(color: AppColors.red400, fontSize: 12)),
                ),
            ],
          ),
        ),
                  ...
                  builder: (context, value, _) {
                    ...
                    return TextButton(
                      ...
                      style: TextButton.styleFrom(
                        backgroundColor: canSend ? AppColors.indigo600 : AppColors.slate800,
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(999)),
                      ),
                      child: Text(
                        'Send',
                        style: TextStyle(color: canSend ? Colors.white : AppColors.slate600, fontSize: 13),
                      ),
                    );
                  },
                ),
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: Container(
        ...
        child: Text(
          message.content,
          style: TextStyle(color: isUser ? Colors.white : AppColors.slate100, fontSize: 14, height: 1.4),
        ),
      ),
    );
  }
